...
2026-09-10 实现补充：分享包顶层目录统一为 `招聘智能体/recruitment-agent` 与 `消息平台/message-platform`。消息平台负责飞书入站、出站、重试、幂等、审计与 Agent 路由；招聘定时策略和 Windows 任务注册属于相邻招聘智能体。消息平台通过新相对路径读取招聘环境和话术模块，首次使用须配置飞书凭据、群白名单与 route，并优先使用官方长连接；HTTP 回调默认只绑定 `127.0.0.1` 且必须鉴权。
