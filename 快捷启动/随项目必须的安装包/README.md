...
项目运行需要 Node.js 20+，消息平台飞书长连接需要 `@larksuiteoapi/node-sdk`；但这些依赖不一定要放在这个目录里。如果电脑本机已经安装了合格的 Node.js/pnpm，可以优先使用本机环境，只把飞书 SDK 依赖安装/修复到快捷启动依赖区即可。
...
- 如果本机已有 Node.js 20+ 和 pnpm：推荐使用本机环境，只安装/修复飞书 SDK 运行依赖。
