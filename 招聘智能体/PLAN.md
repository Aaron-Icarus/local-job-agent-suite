...
- 入口：`recruitment-agent/run_daily_job_agent.ps1 -Scheduled`。时间策略集中在 `recruitment-agent/config/schedule_policy.json`；Windows 主任务为 14 个独立时间点，另有 21:05 独立 watchdog。所有任务不唤醒电脑、错过不补。
- 采集：上午 09:00–13:00、下午 13:00–19:00 各最多成功一次，每小时有独立触发点，两次成功至少相隔 2 小时；19:00–21:00 每半小时尝试日报。当天 BOSS 或猎聘任一平台失败时，不会回退使用历史文件。
- 平台开关：`recruitment-agent/config/platform_channels.json` 独立控制 BOSS/猎聘整条登录、采集、筛选、评分、状态刷新和日报链路；BOSS 详情采集默认开启。
...
- 晚间重试：19:00–21:00 日报只有 `success` 会封版；`partial_success` 或 `failed` 会按 `schedule_policy.json` 的 `retry_gap_minutes` 在窗口内重试，默认 30 分钟。
