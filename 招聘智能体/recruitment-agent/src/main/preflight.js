...
async function main() {
  ...
  if (result.ok) {
    console.log(JSON.stringify({ type: "preflight", ok: true, warnings: result.warnings || [] }));
    return;
  }
  ...
  console.error(JSON.stringify({ type: "preflight_failed", message: "启动前配置检查未通过", issues: result.issues, warnings: result.warnings || [] }));
  process.exitCode = 2;
}
