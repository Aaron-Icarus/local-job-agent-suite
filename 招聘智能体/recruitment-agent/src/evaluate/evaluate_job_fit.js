...
function scoreSalary(row, reasons, risks) {
  const targetMin = numberOr(profileRules.targetSalaryK?.min, 20);
  const targetMax = numberOr(profileRules.targetSalaryK?.max, 25);
  const min = optionalNumber(row.salary_min_k);
  const max = optionalNumber(row.salary_max_k);
...
function scoreActivity(row, reasons, risks) {
  const highDays = numberOr(profileRules.activityRules?.activeWithinDaysHigh, 7);
  const lowDays = numberOr(profileRules.activityRules?.activeWithinDaysLow, 14);
  const ignoreDays = numberOr(profileRules.activityRules?.inactiveOverDaysIgnore, 30);
  const activeText = val(row.boss_active_text || row.recruiter_active_text || row.active_text);
  const activeDays = optionalNumber(row.active_days);
...
function scoreDetailConfidence(row, reasons, risks) {
  ...
  if (row.collection_status === "list_api_only") {
    risks.push("当前使用 BOSS 列表字段，未自动打开岗位详情；可作为初步候选，但投递前需人工确认完整职责");
    return 55;
  }
