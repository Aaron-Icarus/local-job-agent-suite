...
// Full detail capture is the normal BOSS mode. Operators can turn it off in
// config/platform_channels.json when they deliberately want list-only mode.
const enableUiDetailCapture = envBool("BOSS_UI_DETAIL_CAPTURE", true);
...
async function clickAndCapture(job, keyword, strategyType) {
  ...
  // openWs() deliberately returns to the stable jobs page after the previous
  // card opened a detail route. Re-apply the current keyword for every card;
  // otherwise only the first card of a keyword can be found reliably.
  let searchState;
  try {
    searchState = await searchUi(ws, keyword, job.jobName);
  } catch (err) {
    ws.close();
    return {
      keyword,
  ...
  const locateExpr = `(async () => {
    ...
    const find = () => {
      const candidates = [...document.querySelectorAll(selector)].map((el) => {
        ...
        const link = el.closest?.('a[href]') || (el.matches?.('a[href]') ? el : null);
  ...
  await sleep(500);
  if (loc.href) {
    // Navigate the existing controlled tab. This keeps Network events on the
    // same CDP target and prevents target=_blank links from leaking tabs.
    const destination = await ws.cmd("Runtime.evaluate", {
      expression: `new URL(${JSON.stringify(loc.href)}, location.href).href`,
      returnByValue: true,
    });
    await ws.cmd("Page.navigate", { url: destination.result?.value }, 15000);
  } else {
...
module.exports = { captureDetailPageSnapshot, extractRecord };
