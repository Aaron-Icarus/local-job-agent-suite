...
默认 `SEND_MODE=draft`，只写草稿，不发送。当前定时晚间推送由 `SCHEDULE_SEND_MODE` 控制；该变量缺失时源码也按 `draft` 处理，只有显式设置为 `send` 才会真实发送。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "Set-Location -LiteralPath '<分享包实际安装目录>\招聘智能体\recruitment-agent'; & '.\run_daily_job_agent.ps1' -DraftOnly"
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "$packageRoot='<分享包实际安装目录>'; $node=Join-Path $packageRoot '快捷启动\随项目必须的安装包\node\node.exe'; if (!(Test-Path -LiteralPath $node)) { $node='node' }; Set-Location -LiteralPath (Join-Path $packageRoot '招聘智能体\recruitment-agent'); & $node '.\src\platforms\boss\check_boss_login_status.js' --new"
```

单独检查猎聘登录态：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "$packageRoot='<分享包实际安装目录>'; $node=Join-Path $packageRoot '快捷启动\随项目必须的安装包\node\node.exe'; if (!(Test-Path -LiteralPath $node)) { $node='node' }; Set-Location -LiteralPath (Join-Path $packageRoot '招聘智能体\recruitment-agent'); & $node '.\src\platforms\liepin\check_liepin_login_status.js' --new"
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "Set-Location -LiteralPath '<分享包实际安装目录>\招聘智能体\recruitment-agent'; & '.\tools\open_liepin_cdp_for_login.ps1'"
```

确认草稿无误后，可手动发送最新草稿：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "Set-Location -LiteralPath '<分享包实际安装目录>\招聘智能体\recruitment-agent'; & '.\run_daily_job_agent.ps1' -SendLatestDraft"
```

创建 Windows 计划任务示例：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -Command "Set-Location -LiteralPath '<分享包实际安装目录>\招聘智能体\recruitment-agent'; & '.\bin\create_windows_task.example.ps1'"
```

主任务名是 `Recruitment Agent Schedule`，日报缺失检查任务名是 `Recruitment Agent Report Watchdog`。注册脚本会先注册并校验这两个新任务，再自动删除已经废弃的 `BOSS Job Agent Daily`，因此项目改名后不会残留指向旧目录的任务地址。
...
- `config/schedule_policy.json`：唯一人工配置入口，存时间段、每个触发时间、成功配额、两次采集间隔、日报重试和 watchdog 时间。
- `src/core/schedule_policy.js`：读取并校验配置，阻止时间重叠、错误格式、唤醒电脑或错过补跑等不符合约定的设置。
- `src/main/scheduled_entry.js`：只负责读取配置、运行决策和记录成功/失败状态，不再硬编码时段。
- `bin/create_windows_task.example.ps1`：为每个时间创建独立 Windows 触发器；不使用依附于 09:00 的重复链。
- `bin/verify_windows_tasks.ps1`：核对 Windows 中的实际任务与 JSON 是否一致。

当前默认规则：

- 上午窗口 09:00-13:00；09:00、10:00、11:00、12:00 独立触发，最多成功采集一次。
- 下午窗口 13:00-19:00；13:00 到 18:00 每小时独立触发，最多成功采集一次。
- 两次成功采集至少相隔 2 小时；失败或 `partial_success` 不占用半天成功配额，下一时间点可重试。
- 日报窗口 19:00-21:00；19:00、19:30、20:00、20:30 独立触发。距最近成功采集满 2 小时就先采集，否则直接汇总。
- 所有触发器都不唤醒电脑、错过不补。比如 09:00 睡眠但 10:00 已开机，10:00 的独立触发器仍可运行；如果 09:00 是唯一触发器加重复链，则做不到这一点。
- 21:05 运行独立 watchdog，检查当日日报、真实发送回执以及 Windows 任务配置；它不依赖招聘主流程是否启动。
- 晚间日报只有 `success` 会封版；`partial_success` 或 `failed` 不封版，满 30 分钟后在窗口内重试。

修改时间时，编辑 `config/schedule_policy.json`，然后重新运行计划任务注册脚本。只改 JSON 不重建 Windows 任务，操作系统仍会保留旧时间。
...
`config/platform_channels.json` 是 BOSS 与猎聘的总开关。`enabled=false` 时，该平台不会参与登录检查、采集、筛选、评分、岗位状态刷新和日报，手工指定输入文件也不能绕过。控制台中的“启用整条任务流”复选框直接保存这个文件。

`detail_capture` 是详情采集设置，不是平台开关。BOSS 默认为 `true`：读取列表后还会打开岗位详情，获得更完整的 JD。设为 `false` 仍能读取列表摘要，但信息不足的岗位会标记为“详情待确认”。两个平台目前共享同一个 Chrome CDP profile，因此浏览器采集有意顺序执行，避免两个并行页面同时导航、抢标签页或误判登录态；岗位库状态刷新已经按平台隔离，一个平台失败不会把另一个平台历史岗位标为失效。若未来要真正并行，应为平台配置不同 Chrome profile 和 CDP 端口，而不只是开两个标签页。

零结果分三种情况：单个关键词 0 条属于正常搜索结果，继续处理其他关键词；所有关键词均为 0 条但平台接口正常时，仍生成一份明确写着“没有可推荐新岗位”的日报；没有当天输入文件、没有开启任何阶段或应生成报告却没有报告产物时属于配置/流程异常，程序会退出并告警。
...
- **2026-09-10（v0.4.1）独立调度与平台总开关版**：时间策略集中到 `config/schedule_policy.json`；Windows 主任务使用 14 个独立时间点，明确不唤醒、不补跑；增加 21:05 独立日报 watchdog；平台总开关集中到 `config/platform_channels.json` 并接入控制台；BOSS 详情采集默认开启；岗位状态刷新按平台隔离；Node.js 最低版本统一为 22.4.0；飞书 HTTP 回调增加 token/签名校验与重放保护。
- **2026-09-10（v0.4.0）调度与空字段语义修复版**：修复空岗位薪资/活跃天数误当作 0、BOSS 列表岗位不能进入“需确认详情”候选，以及无处理阶段或无当天输入静默成功等问题。其早期计划任务方案已被 v0.4.1 的独立触发器替代。
- **2026-09-08（v0.3.5）采集重入与 BOSS 非侵入式采集修复版**：增加工作流互斥锁、登录/采集超时和可选列表模式。当时采用的 16:00 补采与“BOSS 详情默认关闭”已被 v0.4.1 的独立触发策略和详情默认开启替代，本条仅保留版本历史。
