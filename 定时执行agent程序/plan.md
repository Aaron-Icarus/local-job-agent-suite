...
> 重要：本文件前半部分保留历史招聘 Agent 设计记录。2026-09-04 后，`定时执行agent程序/` 当前承载的是**消息发送转发与智能体调度平台**，可运行代码在 `message-platform/`；招聘采集、Boss/猎聘日报、关键词、评分和告警逻辑在 `../AI消息群聊转发agent/recruitment-agent/`。后续 AI 对话读取本文件时，应以后文“2026-09-04 当前执行说明”为准。
...
- **2026-09-04 v0.3.0**：项目职责迁移完成后，本目录定位为消息发送转发与智能体调度平台；招聘采集与日报逻辑迁移到相邻 `AI消息群聊转发agent/recruitment-agent/`。
