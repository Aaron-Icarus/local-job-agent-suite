...
function decide(state, now = new Date()) {
  ...
  if (slot === "delivery") {
    if (hasFinalizedReportAttempt(state, parts.dateKey)) {
      return {
        shouldRun: false,
        action: "skip",
        slot,
        dateKey: parts.dateKey,
        reason: hasSuccessfulEveningReport(state, parts.dateKey) ? "daily report already delivered" : "daily report failed; carry data forward for next-day revalidation"
      };
    }
    if (!lastToday) {
      return {
