...
function jsonArray(value) { return Array.isArray(value) ? value.map(String).filter(Boolean).slice(0, 6) : null; }

function compactJob(row, index) {
  return { index, platform: row.platform || "", company: row.company || "", job_title: row.job_title || "", salary: row.salary || "", location: row.district || row.city || "", job_text: String(row.job_description || row.page_card_text || "").slice(0, 2500), collection_status: row.collection_status || "" };
}
...
async function refineEvaluations(records, baselineRows, profileRules) {
  ...
  return baselineRows.map((baseline, index) => {
    ...
    return { ...baseline, evaluation_version: "fit_eval_v3_ai_router", overall_fit_score: overall == null ? baseline.overall_fit_score : overall, application_success_score: success == null ? baseline.application_success_score : success, focus_level: item.focus_level || baseline.focus_level, application_recommendation: String(item.application_recommendation || baseline.application_recommendation).slice(0, 160), evaluation_summary: String(item.evaluation_summary || baseline.evaluation_summary).slice(0, 500), match_reasons_json: JSON.stringify(jsonArray(item.match_reasons) || JSON.parse(baseline.match_reasons_json || "[]")), risk_reasons_json: JSON.stringify(jsonArray(item.risk_reasons) || JSON.parse(baseline.risk_reasons_json || "[]")), ai_evaluation_status: "success", ai_evaluation_provider: ai.provider, ai_evaluation_model: ai.model, ai_evaluation_reason: ai.fallback ? "步骤配置失败后使用默认 AI 配置" : "使用步骤配置或默认 AI 配置" };
  });
}
