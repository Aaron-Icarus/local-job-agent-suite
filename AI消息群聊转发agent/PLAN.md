...
岗位打招呼提示词存放于 `prompts/job_greeting_agent.md`，其输入由 `src/prompts/job_greeting_prompt.js` 组装：用户真实指令、岗位编号对应的结构化岗位事实、候选人事实和会话摘要。提示词要求按“岗位识别行 + 100 字内 HR 打招呼文案”输出，且不得虚构营销、海外或研发经历。

这一层目前只完成输入契约和提示词构造，尚未把 Codex/CDH/API 调用器接入执行器；规则版执行器继续用于离线链路测试。
...
- 状态：支持 `success`、`partial_success`、`failed`。单平台失败会告警并保留另一平台的当日结果；不会把部分成功记成完整成功。
