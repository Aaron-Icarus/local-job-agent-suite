...
> 重要：本文件前半部分保留历史招聘 Agent 设计记录。2026-09-04 后，`消息平台/` 当前承载的是**消息发送转发与智能体调度平台**，可运行代码在 `message-platform/`；招聘采集、Boss/猎聘日报、关键词、评分和告警逻辑在 `../招聘智能体/recruitment-agent/`。后续 AI 对话读取本文件时，应以后文“2026-09-04 当前执行说明”为准。

## 构建包版本更新说明

- **2026-09-10 v0.4.2**：顶层目录改名为 `招聘智能体`、`消息平台`；消息路由、环境文件和招聘话术模块的相对路径已经同步，定时策略和 Windows 任务仍由相邻招聘智能体负责。
...
- **2026-09-07 v0.3.1**：按新用户模拟流程复核分享包。消息平台代码与模拟目录保持一致，发送入口在飞书凭据缺失时立即给出配置错误，不做无意义三次重试；发送成功会记录幂等键、尝试次数和平台返回 message_id；测试用例使用示例岗位/规则话术，不依赖真实岗位库或真实 AI。招聘侧同步修复 `AI_GREETING_MODE`、BOSS 强登录校验和 partial 阻断，消息平台只负责发送/路由/审计，不判断招聘隐私和岗位匹配。
- **2026-09-04 v0.3.0**：项目职责迁移完成后，本目录定位为消息发送转发与智能体调度平台；招聘采集与日报逻辑迁移到相邻 `招聘智能体/recruitment-agent/`。
...
- 电脑开着时执行，不要求关机后补跑。
- 09:00、10:00、11:00、12:00 分别独立触发；上午最多成功一次。
- 13:00 到 18:00 每小时分别独立触发；下午最多成功一次。
- 19:00、19:30、20:00、20:30 分别独立触发汇总推送，21:05 独立检查日报是否缺失。
- 所有触发器不唤醒电脑、错过不补；时间策略以相邻招聘项目的 `config/schedule_policy.json` 为准。
...
注意：本文件保留了早期合并方案的内容。当前可执行的招聘调度代码实际位于相邻的 `招聘智能体/recruitment-agent`；本目录下的 `message-platform` 只负责消息收发、重试、审计和群聊 Agent 路由，不负责决定何时采集岗位或生成日报。
...
话术推荐历史上曾按 `GREETING_MODE` 单独控制；当前已统一改为招聘 Agent 的 `src/core/ai_router.js`。候选人画像、搜索关键词、岗位评分复核和打招呼话术共享 `AI_DEFAULT_*` 与各步骤 `AI_<用途>_*` 路由，分享包默认关闭 AI；启用后可优先本机 Codex，OpenAI 兼容 API 作为可选兜底。AI 不可用时，搜索/评分/话术按各自规则降级，画像生成则明确报错。
...
主任务名是 `Recruitment Agent Schedule`，日报缺失检查任务名是 `Recruitment Agent Report Watchdog`。注册脚本会先校验新任务，再删除已废弃的 `BOSS Job Agent Daily`，避免保留指向旧目录的注册地址。
...
本目录当前承载**消息发送转发与智能体调度平台**，可运行工程位于 `message-platform/`；招聘采集和日报编排属于 `../招聘智能体/recruitment-agent/`。
...
- 告警：Boss/猎聘采集失败、部分成功、19:00–21:00 日报补跑等逻辑均在招聘 Agent 侧判断；消息平台只负责把招聘 Agent 生成的告警文本按幂等键发送，并记录成功、失败耗尽或不确定送达。
- 分享包状态：构建包默认关闭采集、发送和 AI；本轮仅同步文档边界和通用代码，不包含任何真实群、凭据、岗位数据或本机绝对路径。
